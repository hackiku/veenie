	...
	function toggleWind() {
		if (sim) {
			...
			const currentlyEnabled =
				conditions.windVector[0] !== 0 || conditions.windVector[2] !== 0;
	...
	<div class="space-y-3">
		...
		<div>
			...
			<div class="w-full">
				<Slider.Root
					...
					max={1}
					step={0.01}
					value={sim ? parseFloat(sim.getBuoyancy().toFixed(2)) : 0.3}
		...
		<div>
			<label for={windIntensityId} class="block text-white/60 text-xs mb-1">
				Wind Intensity: {sim
					? sim.getAtmosphericConditions().windVector[0].toFixed(1)
					: "1.0"}
			</label>
			<div class="w-full">
				<Slider.Root
					...
					max={2}
					step={0.10}
					value={sim?.models?.atmosphere?.getWindIntensity() || 1.0}
